...
\caption{Psevdokoda programa}
 ...
 \KwResult{M, nmg // M-meja za avtoriziranje, nmg-namig avtorizantu}
	...
	// s $|P_n|$ oznaèimo moè populacije $P_n$, $m$ = prièakovano število popustov, $A$ = množica vseh nakljuènih populacij\;
  $\mu$=mean($P_u$)+$\mu_d$\;
	$\sigma$=std($P_u$)+$\sigma_d$\;
	$M_z:=\mu+\sigma$ // zaèetna meja\;
	$k:=1$ // koeficient za premikanje meje\;
	\For{i:=1 \textbf{to} $stPop$}{
	generiraj $P_s$ velikosti $m$ z $\mu$ in $\sigma$ in jo dodaj v $A$\;
	}
	$P_r:=\emptyset$\;
	$P_n:=\emptyset$\;
	\line(1,0){50} \\
	...
	$M:=k\cdot M_z$ // nova trenutna meja\;
