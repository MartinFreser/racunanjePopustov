...
generirajModel <- function(datumOd, datumDo, datumOd2, datumDo2, privzetaMejaZaAvtorizacijo, dPovprecje, dSD, dAvtorizacije = 0, indeksRasti = 1,stNakljucnihPopulacij = 1, mesto = NULL){
	...
	#c(grafi,zp$DODELJENI_POPUSTI_952_ODS[gledaniPodatki])
	hist(zp$DODELJENI_POPUSTI_952_ODS[gledaniPodatki])
	...
	for (i in 1:stNakljucnihPopulacij){
		celotnaPopulacija[[i]] <- simuliranaPopulacija[[i]] <- skew.rnorm(stPodatkov, povZp, sdZp, skewness(zp$DODELJENI_POPUSTI_952_ODS[gledaniPodatki], na.rm=TRUE)) #generiramo nakljucno populacijo, celotna je na začetku naključna
		if(i==1){ 
			#c(grafi,celotnaPopulacija[[i]]))
		}
	 }
